...
inline bool plausibleSpan(Vec3 a, Vec3 b) {
    if (!finite3(a) || !finite3(b)) return false;
    const float span = distance(a, b);
    return span >= kMinimumSpan && span <= kMaximumSpan;
}
...
inline ChainSnapshot preview(Vec3 origin, const TargetSample& sample, Verdict verdict) {
    ...
    if (verdict == Verdict::Pending || verdict == Verdict::Miss ||
        ...
        return snap;
    if (!plausibleSpan(origin, sample.position)) return snap;
    snap.visible = true;
...
inline ChainSnapshot launchSnapshot(Vec3 origin, const LaunchState& s) {
    ChainSnapshot snap{};
    const Vec3 tip = launchTip(s);
    if (!plausibleSpan(origin, tip)) return snap;
    snap.visible = true;
    snap.nearPoint = origin;
    snap.farPoint = tip;
...
inline ChainSnapshot latched(Vec3 origin, Vec3 anchor) {
    ChainSnapshot snap{};
    if (!plausibleSpan(origin, anchor)) return snap;
    snap.visible = true;
    snap.nearPoint = origin;
