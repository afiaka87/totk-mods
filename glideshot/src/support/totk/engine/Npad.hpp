...
public:
    ...
    // Masks only the fresh raw states of this frame: a mod suppresses only the input it owns.
    ...
    // Overwrites the left-stick axes of this frame's fresh states only.
    ...
    // Same scoped substitution for the right stick.
    ...
    // Adds an owned synthetic button edge to the same fresh samples; gate it to an explicit player
    // action.
