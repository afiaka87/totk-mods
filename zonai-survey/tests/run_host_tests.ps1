...
& uv run --no-project python (Join-Path $testsDir "surface/test_shader_bindings.py")
...
$exe = Get-ChildItem -Path $buildDir -Recurse -Filter 'zonai_survey_pure_tests*' -File |
